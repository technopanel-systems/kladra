...
| Export | on every list screen | Can I have this list as a file? | Not a screen: an outline button beside the screen's own action, which hands back the list as it stands — the search, the chip and the drill-down in the address included (P14 14.10). A panel of files was the old shape, and it was a second answer to "which rows". |
