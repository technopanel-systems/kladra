...
function CompanyForm({
  ...
  const [countryId, setCountryId] = useState(lookups.saudiCountry ?? "");
  const [cityId, setCityId] = useState(lookups.defaultCity ?? "");
  ...
  useEffect(() => {
    if (state?.ok && state.data) onCreated(state.data.companyId, submitted.current);
  }, [state, onCreated]);
  ...
  function pickCountry(next: string) {
    setCountryId(next);
    // Anywhere but Saudi Arabia the city is free text, and the picked city goes
    // with the country it belonged to (SPEC §3).
    if (lookups.saudiCountry !== null && next === lookups.saudiCountry) {
      setCityText("");
      setCityId(lookups.defaultCity ?? "");
    } else {
      setCityId("");
    }
  }
  ...
  const warning =
    duplicate === null ? null : (
      ...
      >
        ...
        <span>
          {t("forms.duplicateCompany", { name: duplicate.name, rep: duplicate.rep })}
        </span>
      </p>
    );
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto overscroll-contain px-4 pb-4">
        <div className="flex flex-col gap-1.5">
          ...
          {duplicate?.matchedOn === "name" ? warning : null}
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="company-category">{t("common.category")}</Label>
            <SearchableSelect
              id="company-category"
              value={categoryId}
              ...
              aria-describedby={errors.categoryId ? "company-category-error" : undefined}
            />
            <input type="hidden" name="categoryId" value={categoryId} />
            {errors.categoryId ? (
              <p id="company-category-error" role="alert" className="text-xs text-destructive">
                {errors.categoryId}
              </p>
            ) : null}
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="company-lead-source">{t("common.leadSource")}</Label>
            <SearchableSelect
              id="company-lead-source"
              value={leadSourceId}
              ...
              aria-describedby={errors.leadSourceId ? "company-lead-source-error" : undefined}
            />
            <input type="hidden" name="leadSourceId" value={leadSourceId} />
            {errors.leadSourceId ? (
              <p id="company-lead-source-error" role="alert" className="text-xs text-destructive">
                {errors.leadSourceId}
              </p>
            ) : null}
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="company-country">{t("common.country")}</Label>
            <SearchableSelect
              id="company-country"
              value={countryId}
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="company-city">{t("common.city")}</Label>
            {inSaudi ? (
              <>
                <SearchableSelect
                  id="company-city"
        ...
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="company-notes">{t("common.notes")}</Label>
          <Textarea
            id="company-notes"
            name="notes"
            rows={2}
