...
description: Reads Kladra's own attack surface — the server actions, the route handlers, the auth bridge and the session cookie — and reports what an attacker could reach. Findings only; it writes nothing.
tools: Read, Grep, Glob, Bash
model: opus
---

<!--
From aitmpl.com's catalogue (agents/security/api-security-audit.md), installed
in P14.5 with two changes: Write and Edit taken away, because one writer per
file is a house rule and the session applies the fixes; and "use PROACTIVELY"
taken out of the description, because this is asked for rather than volunteered.
The catalogue's other two security agents (security-auditor, security-engineer)
were read and left: both are written for SOC 2, PCI DSS, Kubernetes and Vault,
and Kladra is one Postgres and one Next.js container for fourteen people.
-->
