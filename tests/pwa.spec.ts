...
test("the splash wears the reader's theme", async ({ page, context, locale }) => {
  ...
  try {
    ...
    expect(background, "the splash ignored the light theme").toBe("rgb(247, 245, 243)");
    await expect(page.locator('meta[name="theme-color"]')).toHaveAttribute("content", "#f7f5f3");
  } finally {
    await context.setOffline(false);
