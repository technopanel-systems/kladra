 ...
 * thinking, not work stopped on the rep. The heading carries the three kinds
 * as pills now, stopped work sorts first, and each pill opens the list under
 * that status. This holds the three together: the pill's number is the cards
 * of that kind (the seeded list is under the cap, so every row is drawn), the
 * first card is stopped work, and the door lands on exactly that many rows.
 *
...
test("the waiting list sorts stopped work first and its pills open their own lists", async ({
  ...
  for (const kind of kinds) {
    ...
    if (n > 0) await expect(pill).toHaveAttribute("href", new RegExp(kind.href));
  }
  expect(counted.sentBack + counted.refused + counted.withCustomer).toBe(drawn);

  // Stopped work first: whichever stopped kind exists, the first card is one of
  // them and no customer-held quotation sits above it.
  const stopped = counted.sentBack + counted.refused;
  if (stopped > 0) {
    const firstBadge = cards.first().locator('[data-slot="state-badge"], span').filter({
      hasText: new RegExp(`^(${t("day.sentBack")}|${t("day.refused")})$`),
    });
